/*
  Autosplitter for Metal Gear Solid 2 (PC)
*/
...
isLoading {
  ...
  if ((vars.LoadlessTimerModel == null) || (vars.LoadlessTimerModel.CurrentState.StartTime != vars.TimerModel.CurrentState.StartTime)) {
    vars.LoadlessTimerModel = new TimerModel { CurrentState = (LiveSplitState)timer.Clone() };
    vars.LastAreaLoadTime = vars.LoadlessTimerModel.CurrentState.CurrentTime.RealTime;
  }

  var loadless = vars.LoadlessTimerModel.CurrentState;
  ...
  if (current.GameplayActive != old.GameplayActive) {
    if (current.GameplayActive == 1)
      ...
      vars.LastAreaTime = (loadless.CurrentTime.RealTime - vars.LastAreaLoadTime).ToString(@"m\:ss\.ff");
      vars.LastAreaLoadTime = loadless.CurrentTime.RealTime;
    }
    vars.LoadlessTimerModel.Pause();
  }

  vars.RTALoadless = loadless.CurrentTime.RealTime.ToString(@"h\:mm\:ss\.ff");

  return ((!settings["rta_loadless"]) || (current.GameplayActive != 1));
}
...
gameTime {
  ...
  return (vars.IsRtaLoadlessEnabled()) ?
    vars.LoadlessTimerModel.CurrentState.CurrentTime.RealTime : gameTime;
}
...
startup {
  ...
  settings.Add("options", true, "Advanced Options");
    ...
    settings.Add("rta_loadless", false, "Change Game Time style to RTA Loadless (MC only)", "options");
...
init {
  vars.M = null;
  vars.IsMasterCollection = false;
  vars.IsSubstance = false;
  vars.TimerModel = new TimerModel { CurrentState = timer };
  vars.LoadlessTimerModel = new TimerModel { CurrentState = (LiveSplitState)timer.Clone() };

  if (game.ProcessName.Equals("mgs2_sse"))
  {
    vars.IsSubstance = true;
    vars.IsRtaLoadlessEnabled = (Func<bool>)(() => false);
    return true;
  }

  vars.NextAttachAttempt = DateTime.Now;
  vars.IsMasterCollection = true;
  vars.IsRtaLoadlessEnabled = (Func<bool>)(() => settings["rta_loadless"]);

  vars.FindReference = (Func<int, string, IntPtr>)((patternOffset, pattern) => {
  ...
  vars.FindRelativePointer = (Func<int, string, int, string, IntPtr>)((patternOffset, pattern, pointerOffset, name) => {
      ...
      */
      ...
      Action UpdateASLVars = delegate() {
        if (current.GameTime != 0) {
          long chkTargetFrames = (((((29*60)+59)*60)+1)*60);
          long chkRemainingFrames = chkTargetFrames - current.GameTime;
          TimeSpan chkRemainingTime = TimeSpan.FromMilliseconds(chkRemainingFrames * 1000 / 60);
          if ((int)chkRemainingTime.TotalMinutes != 0) {
            vars.ASL_ChickenTimeLeft = string.Format(
              ((int)chkRemainingTime.TotalHours != 0) ? @"{1}:{0:mm\:ss}" : @"{0:m\:ss}", chkRemainingTime, (int)chkRemainingTime.TotalHours); 
          }
          else {
            vars.ASL_ChickenTimeLeft = string.Format(@"{0:s\.ff}", chkRemainingTime);
          }
        }
    ...
    if (vars.IsMasterCollection) {
      ...
      if ((vars.M == null) && (DateTime.Now >= vars.NextAttachAttempt))
      {
        ...
        if (pointers["gameplayActive"] == IntPtr.Zero)
          pointers["gameplayActive"] = vars.FindRelativePointer(7,
            "E8 ?? ?? ?? 00 83 3D ?? ?? ?? 00 00 74 ?? 0F 2F 05 ?? ?? ?? ?? 73 ?? 33 C0 48 83 C4 28 C3", 5, "MC2 gameplay active status (1.0)");

        pointers.Add("mainData", vars.FindRelativePointer(11, "81 F9 34 03 00 11 75 ?? 48 8B 05 ?? ?? ?? 00", 4, "MC2 main game data"));
        ...
        vars.NextAttachAttempt = DateTime.Now.AddYears(420);
        ...
        vars.M = new MemoryWatcherList()
        {
          ...
          new StringWatcher(
            new DeepPointer(pointers["mainData"], 0x2C), 10) { Name = "RoomCode" },
      ...
      cur["OlgaRushStamina"] = (byte)0;
      cur["MerylHealth"] = (byte)0;
      cur["FatmanHealth"] = (short)0;
      cur["FatmanStamina"] = (int)0;
      cur["FatmanBombsActive"] = (byte)0;
      cur["HarrierHealth"] = (short)0;
      cur["VampStamina"] = (short)0;
      cur["VampHealth"] = (short)0;
      cur["SolidusHealth"] = (byte)0;
      cur["SolidusStamina"] = (byte)0;
      cur["Vamp2Health"] = (short)0;
      cur["Vamp2Stamina"] = (short)0;
      cur["Tengus2Defeated"] = (short)0;
      cur["RaysHealth"] = (short)0;
      ...
      cur["EmmaHealth"] = (byte)0;
      cur["VRMissionID"] = (short)0;
      cur["VRCurrentScore"] = (int)0;
    }
  }
  catch (Exception e) {
